...
export class MacroTransformer {
  ...
  visitor(node: ts.Node): ts.VisitResult<ts.Node> {
    ...
    if (
      ...
      if (ts.isNonNullExpression(node.expression.expression)) {
        const statements = this.runMacro(
          node.expression,
          node.expression.expression.expression,
        )
        if (!statements) return
        const prepared = this.makeHygienic(
    ...
    if (ts.isCallExpression(node)) {
      if (ts.isNonNullExpression(node.expression)) {
        ...
        ) as unknown as Array<ts.Statement> | undefined
    ...
    if (this.macroStack.length) {
      ...
      if (
        ...
        if (
          ...
          let exp: ts.Expression = ts.visitNode(
            node.expression,
            this.boundVisitor,
          )
          ...
          if (ts.isObjectLiteralExpression(exp)) {
            const name = ts.isPropertyAccessExpression(node)
              ...
              : this.getNumberFromNode(
                  ts.visitNode(node.argumentExpression, this.boundVisitor),
                )
            if (!name) return node
      ...
      // Detects a ternary expression and tries to remove it if possible
        ...
        if (res === false)
          ...
          return ts.factory.createConditionalExpression(
            param,
            undefined,
            ts.visitNode(node.whenTrue, this.boundVisitor),
            undefined,
            ts.visitNode(node.whenFalse, this.boundVisitor),
          )
      }
      ...
      // Detects an if statement and tries to remove it if possible
        ...
        return ts.factory.createIfStatement(
          condition,
          ts.visitNode(node.thenStatement, this.boundVisitor),
          ts.visitNode(node.elseStatement, this.boundVisitor),
        )
      }
      ...
      // Detects a unary expression and tries to remove it if possible
        ...
        const value: ts.Expression = ts.visitNode(
          node.operand,
          this.boundVisitor,
        )
      ...
      // Detects a typeof expression and tries to remove it if possible
        ...
        const val = this.getLiteralFromNode(
          ts.visitNode(node.expression, this.boundVisitor),
        )
        if (val === NO_LIT_FOUND) return node
      ...
      // Detects a repetition
        ...
        const {
          ...
          function: fn,
          literals,
        } = getRepetitionParams(node.expression.operand)
        ...
        const {
          ...
          function: fn,
          literals,
        } = getRepetitionParams(node.operand)
        if (!separator)
          throw MacroError(
            node,
        ...
        if (repNodeIndex !== -1) {
          ...
          const {
            ...
            function: fn,
            literals,
          } = getRepetitionParams(repNode)
          if (!separator) {
            const newBod = this.execRepetition(
              fn,
              args,
              macro,
              literals,
    ...
    for (
      ...
      if ("statements" in fn.body) {
        if (wrapStatements)
          newBod.push(
            wrapExpressions(
              fn.body.statements.map((node) =>
                ts.visitNode(node, this.boundVisitor),
              ),
            ),
          )
        else {
  ...
  runMacro(
    ...
    if (ts.isPropertyAccessExpression(name)) {
      ...
      const newArgs = ts.factory.createNodeArray([
        ts.visitNode(name.expression, this.boundVisitor),
        ...call.arguments,
      ])
      normalArgs = this.macroStack.length
        ? ts.visitNodes(newArgs, this.boundVisitor)
        : newArgs
    } else {
      ...
      if (nativeMacro) {
        const macroResult = nativeMacro.call(
          nativeMacro.preserveParams
            ? args
            : ts.visitNodes(args, this.boundVisitor),
          this,
          call,
        )
      ...
      if (param.marker === MacroParamMarkers.Save) {
        const value = param.spread
          ? ts.factory.createArrayLiteralExpression(
              normalArgs.slice(param.start),
            )
          : normalArgs[param.start] || param.defaultVal
    ...
    if (ts.isParenthesizedExpression(node))
      ...
      for (const span of node.templateSpans) {
        const lit = this.getLiteralFromNode(
          ts.visitNode(span.expression, this.boundVisitor),
        )
  ...
  getTypeParam(type: ts.Type): ts.Type | undefined {
    ...
    const resolvedTypeParameterIndex = lastMacroCall.macro.typeParams.findIndex(
    ...
    if (lastMacroCall.call) {
      ...
      if (!resolvedTypeParam)
        return this.checker
          .getResolvedSignature(lastMacroCall.call)
          ?.getTypeParameterAtPosition(resolvedTypeParameterIndex)
    ...
    mainLoop: for (const [sym, macro] of this.macros) {
      ...
      const fnArgs = fnType.parameters.map((p) => {
        ...
        if (type.isTypeParameter())
          return type.getConstraint() || this.checker.getAnyType()
        else return type
      })
...
const separators: Record<
  ...
  (
    ...
    body: Array<ts.Expression | ts.Statement>,
  ) => ts.Expression
> = {
  "[]": (_transformer, body) =>
    ts.factory.createArrayLiteralExpression(
