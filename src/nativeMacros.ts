...
export default {
  ...
  $$inlineFunc: {
    call: (args, transformer, callSite) => {
      ...
      if (!fnParam)
        throw MacroError(
          callSite,
          "`inlineFunc` macro expects an arrow function as the first argument.",
        )
      ...
      if (ts.isBlock(newFn))
        return transformer.context.factory.createImmediatelyInvokedArrowFunction(
          newFn.statements,
        )
      return newFn
    },
  ...
  $$kindof: {
    call: (args, transformer, callSite) => {
      if (!args.length)
        throw MacroError(callSite, "`kindof` macro expects a single argument.")
    ...
    call: ([array, item], transformer, callSite) => {
      ...
      if (strContent) {
        ...
        const normalArr = array.elements.map((el) =>
          transformer.getLiteralFromNode(
            ts.visitNode(el, transformer.boundVisitor),
          ),
        )
  ...
  $$ts: {
    call: ([code], transformer, callSite) => {
      const str = transformer.getStringFromNode(
        ts.visitNode(code, transformer.boundVisitor),
        true,
        true,
      )
      if (!str)
  ...
  $$escape: {
    call: ([code], transformer, callSite) => {
      if (!code)
        throw MacroError(
          callSite,
          "`escape` macro expects an arrow function as it's first argument.",
        )
      const maybeFn = ts.visitNode(code, transformer.boundVisitor)
      if (!ts.isArrowFunction(maybeFn))
        throw MacroError(
          callSite,
          "`escape` macro expects an arrow function as it's first argument.",
        )
      if (ts.isBlock(maybeFn.body)) {
        const hygienicBody = [
          ...transformer.makeHygienic(maybeFn.body.statements),
        ]
        ...
        transformer.escapeStatement(
          ts.factory.createExpressionStatement(maybeFn.body),
        )
      }
    },
  },
  ...
  $$propsOfType: {
    call: (_args, transformer, callSite) => {
      if (!callSite.typeArguments || !callSite.typeArguments[0])
        throw MacroError(
          callSite,
          "`propsOfType` macro expects one type parameter.",
        )
      const type = transformer.checker.getTypeAtLocation(
        callSite.typeArguments[0],
      )
      if (type.isTypeParameter()) {
        ...
        if (lastMacro) {
          const allParams = lastMacro.macro.typeParams.map((p) =>
            transformer.checker.getTypeAtLocation(p),
          )
          const replacementTypes = resolveTypeArguments(
            transformer.checker,
            lastMacro.call as ts.CallExpression,
          )
          return ts.factory.createArrayLiteralExpression(
            resolveTypeWithTypeParams(type, allParams, replacementTypes)
              .getProperties()
              .map((sym) => ts.factory.createStringLiteral(sym.name)),
          )
        } else
      ...
      if (type.isTypeParameter()) {
        ...
        return ts.factory.createStringLiteral(
          transformer.checker.typeToString(
            simplify ? transformer.checker.getApparentType(param) : param,
          ),
        )
      } else {
        const lastMacro = transformer.getLastMacro()
        if (lastMacro) {
          ...
          return ts.factory.createStringLiteral(
            transformer.checker.typeToString(
              simplify
                ? transformer.checker.getApparentType(
                    resolveTypeWithTypeParams(
                      type,
                      allParams,
                      replacementTypes,
          ...
          return ts.factory.createStringLiteral(
            transformer.checker.typeToString(
              simplify ? transformer.checker.getApparentType(type) : type,
            ),
          )
      }
    },
  ...
  $$comptime: {
    call: (_, transformer, callSite) => {
      ...
      if (!fn || !ts.isArrowFunction(fn))
        throw MacroError(
          callSite,
          "`comptime` macro expects an arrow function as the first parameter.",
        )
      ...
      if (ts.isExpressionStatement(parent)) {
        ...
        if ("body" in parent) {
          ...
          transformer.comptimeSignatures.set(
            signature.declaration,
            new Function(
              ...signature.parameters.map((p) => p.name),
              fnBodyToString(transformer.checker, fn),
      ...
      tryRun(
        new Function(fnBodyToString(transformer.checker, fn)) as (
          ...args: Array<unknown>
        ) => void,
      )
    },
  ...
  $$raw: {
    call: ([fn], transformer, callSite) => {
      ...
      if (!fn || !ts.isArrowFunction(fn))
        throw MacroError(
          callSite,
          "`raw` macro expects an arrow function as the first parameter.",
        )
      const lastMacro = transformer.getLastMacro()
      if (!lastMacro)
        throw MacroError(
  ...
  $$setStore: {
    call: ([key, value], transformer, callSite) => {
      if (!ts.isStringLiteral(key))
        throw MacroError(
          callSite,
          "`setStore` macro expects a string literal as the key.",
        )
      const lastMacro = transformer.getLastMacro()
      if (!lastMacro)
        throw MacroError(
          callSite,
          "`setStore` macro must be called inside another macro.",
        )
  ...
  $$getStore: {
    call: ([key], transformer, callSite) => {
      if (!ts.isStringLiteral(key))
        throw MacroError(
          callSite,
          "`getStore` macro expects a string literal as the key.",
        )
      const lastMacro = transformer.getLastMacro()
      if (!lastMacro)
        throw MacroError(
          callSite,
          "`getStore` macro must be called inside another macro.",
        )
