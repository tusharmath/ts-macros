...
export function MacroError(callSite: ts.Node, msg: string): void {
  MacroErrorWrapper(
    callSite.pos,
    callSite.end - callSite.pos,
    msg,
    callSite.getSourceFile(),
  )
  process.exit()
}

export function MacroErrorWrapper(
  start: number,
  end: number,
  msg: string,
  file: ts.SourceFile,
...
export function getNameFromBindingName(
  obj: ts.BindingName,
): string | undefined {
...
export function isStatement(obj: ts.Node): obj is ts.Statement {
  return (
    obj.kind >= ts.SyntaxKind.Block &&
    obj.kind <= ts.SyntaxKind.MissingDeclaration
  )
}
  ...
  const visitor = (node: ts.Node) => {
    if (ts.isCallExpression(node)) {
      ...
      if (
        ...
        code += `function ${name}(${signature.parameters
          .map((p) => p.name)
          .join(",")}){${fnBodyToString(checker, signature.declaration)}}`
...
export function tryRun(
  ...
  try {
    ...
    if (err instanceof Error) {
      const {line, col} =
        (err.stack || "").match(/<anonymous>:(?<line>\d+):(?<col>\d+)/)
          ?.groups || {}
...
export function resolveTypeWithTypeParams(
  ...
  // Conditional type
    ...
    if (providedType.checker.isTypeAssignableTo(checkType, extendsType))
      return trueType
  ...
  // Intersections
    ...
    return providedType.checker.createAnonymousType(
      undefined,
      symTable,
      [],
      [],
